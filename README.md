...
| [`subagent.ts`](extensions/subagent.ts) | `subagent` tool: runs one delegated task at a time in an attachable tmux-backed `pi` child; `pi --attach-subagent <id>` takes over the live session (source: [mitsuhiko/agent-stuff](https://github.com/mitsuhiko/agent-stuff), patched to resolve tmux window/pane ids instead of assuming `:0.0`) |
