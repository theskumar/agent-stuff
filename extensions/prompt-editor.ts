...
async function selectModeUI(pi: ExtensionAPI, ctx: ExtensionContext): Promise<void> {
  ...
  while (true) {
    ...
    const items = [...names, MODE_UI_CONFIGURE];
    const initialIndex = Math.max(0, items.indexOf(runtime.currentMode));
    const choice = await pickModeWithDigitsUI(
      ctx,
...
/**
 * Mode picker with numeric quick-select.
 *
 * Each visible entry is numbered (1-9). Pressing the digit instantly selects
 * that entry. Items beyond the 9th get no number prefix and must be reached
 * with the arrow keys.
 *
 * Falls back to plain arrow-key navigation + Enter / Esc otherwise. We use a
 * `ctx.ui.custom` component because `ctx.ui.select` does not let us intercept
 * digit keys.
 */
async function pickModeWithDigitsUI(
  ctx: ExtensionContext,
  title: string,
  items: string[],
  initialIndex: number,
  ...
  return ctx.ui.custom<string | undefined>((tui, theme, _kb, done) => {
    ...
    function handleInput(data: string) {
      ...
      // Digit quick-select: 1-9 maps to the first nine items.
      const maxDigit = Math.min(9, items.length);
      for (let i = 1; i <= maxDigit; i += 1) {
        if (matchesKey(data, String(i) as "1")) {
          done(items[i - 1]);
          return;
        }
      }
...
async function configureModesUI(pi: ExtensionAPI, ctx: ExtensionContext): Promise<void> {
  ...
  while (true) {
    await ensureRuntime(pi, ctx);
    const names = orderedModeNames(runtime.data.modes);
    const items = [...names, MODE_UI_ADD, MODE_UI_BACK];
    const choice = await pickModeWithDigitsUI(ctx, "Configure modes", items, 0);
...
/**
 * ModelSelectorComponent's constructor takes a ModelRuntime
 * (getModel/getAvailableSnapshot/getError/refresh) as its 3rd arg on pi >= 0.85.
 * ctx only exposes modelRegistry, a thin facade wrapping a `runtime` (the
 * ModelRuntime the built-in selector uses); reuse it when present, otherwise
 * fall back to the registry itself for older pi.
 */
...
class PromptEditor extends CustomEditor {
  ...
  render(width: number): string[] {
    ...
    // including the embedded working spinner and any scroll indicator ("↑ N more").
    // When neither is present the whole top row is dashes, so fall back to a two-cell
    // border before the mode label.
    const trailingBorderStart = topPlain.search(/─+$/);
    const semanticPrefix = trailingBorderStart > 0 ? topPlain.slice(0, trailingBorderStart) : "";
    const prefixWidth = Math.min(width, semanticPrefix ? visibleWidth(semanticPrefix) : 2);
    const left = truncateToWidth(topBorder, prefixWidth, "");
    const leftWidth = prefixWidth;
    const leftPlain = topPlain.slice(0, prefixWidth);
    ...
    const labelLeftSpace = leftPlain.endsWith(" ") ? "" : " ";
    const labelRightSpace = " ";
    const minRightBorder = 1; // keep at least one border cell on the right
    const maxLabelLen = Math.max(
      0,
      width - leftWidth - labelLeftSpace.length - labelRightSpace.length - minRightBorder,
    );
    ...
    const remaining = width - leftWidth - labelChunkLen;
