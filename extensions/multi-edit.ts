...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx: ExtensionContext) {
      ...
      if (edits.length === 0) {
        throw new Error("No edits provided. Supply edits[], legacy oldText/newText, or patch.");
      }
      ...
      const resolved = edits.map((e, i) => {
        ...
        if (!display) {
          throw new Error(`Edit ${i + 1} has no path. Set a per-item path or a top-level path.`);
        }
        return {
      ...
      for (const g of groups.values()) {
        ...
        summaries.push(extractText(result.content as any));
        const details = result.details as { diff?: string; firstChangedLine?: number } | undefined;
...
function normalizeForBuiltinRender(params: Params): {
  path: string;
  ...
  const path = params.path ?? edits.find((e) => e.path)?.path ?? "";
  return {
    path,
