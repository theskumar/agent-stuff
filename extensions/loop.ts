 ...
 *
 * What it is:
 *   `/loop` puts pi into a follow-up loop: after every `turn_end`, the
 *   extension re-injects a configured prompt as the next user message and
 *   keeps the agent working until a breakout condition is satisfied. The
 *   agent signals success by calling the `signal_loop_success` tool the
 *   extension registers; once called, the loop ends.
 *
 *   Three modes:
 *     - tests   — keeps running `<test command>` and feeding back failures
 *                 until tests pass.
 *     - custom  — user supplies an arbitrary condition + prompt; loop ends
 *                 when the agent decides `signal_loop_success`.
 *     - self    — agent fully drives the loop (decides when to stop).
 *
 *   Source: adapted from mitsuhiko/agent-stuff.
 *
 * Use cases:
 *   - Drive a "make the tests pass" or "fix all type errors" cycle without
 *     manually re-prompting after each failed attempt.
 *   - Autonomous refactors where you describe the end state and want the
 *     agent to iterate until it claims completion.
 *   - Combined with `/goal` for budgeted long-running tasks.
 *
 * Common usage patterns:
 *   - `/loop` — open the picker, choose a preset (tests / custom / self).
 *   - At the prompt for the test command (`tests` mode), enter e.g.
 *     `pnpm test` or `pytest -q`.
 *   - In `custom` mode, describe the breakout condition + the prompt to
 *     re-send each turn.
 *   - To stop early: have the agent call `signal_loop_success`, or interrupt
 *     pi (Esc/Ctrl+C).
 */
