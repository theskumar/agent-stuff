...
const deletePath = async (
  ...
  const confirmed = await confirmDelete(ctx, target);
  if (!confirmed) {
  ...
  if (process.platform === "darwin") {
    ...
    ctx.ui.notify(`Moved ${target.displayPath} to Trash`, "info");
    return true;
  }

  const trashResult = await pi.exec("gio", ["trash", target.resolvedPath]);
  if (trashResult.code === 0) {
    ctx.ui.notify(`Moved ${target.displayPath} to Trash`, "info");
    return true;
  }
  ...
  ctx.ui.notify(`Deleted ${target.displayPath}`, "info");
  return true;
};
...
const runFileBrowser = async (
  ...
  let { files, gitRoot } = await buildFileEntries(pi, ctx);
  if (files.length === 0) {
  ...
  while (true) {
    ...
    switch (action) {
      ...
      case "diff":
        ...
        break;
      case "delete": {
        const deleted = await deletePath(pi, ctx, selected);
        if (deleted) {
          const refreshed = await buildFileEntries(pi, ctx);
          files = refreshed.files;
          gitRoot = refreshed.gitRoot;
