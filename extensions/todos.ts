...
function buildRefinePrompt(todoId: string, title: string): string {
  return `let's refine task ${formatTodoId(todoId)} "${title}": Ask me for the missing details needed to refine the todo together. Do not rewrite the todo yet and do not make assumptions. Ask clear, concrete questions and wait for my answers before drafting any structured description.\n\n`;
}
...
export default function todosExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    name: "todo",
    label: "Todo",
    description: `Manage file-based todos in ${todosDirLabel} (list, list-all, get, create, update, append, delete, claim, release). Title is the short summary; body is long-form markdown notes (update replaces, append adds). Todo ids are shown as TODO-<hex>; id parameters accept TODO-<hex> or the raw hex filename. Claim tasks before working on them to avoid conflicts, and close them when complete.`,
