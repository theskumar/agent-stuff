 ...
 *
 * What it is:
 *   A side-channel chat popover (`/btw`) that runs alongside the main
 *   conversation. It seeds itself with the main session's context, so the side
 *   agent can answer focused questions without polluting (or being constrained
 *   by) the primary thread. On close, the side conversation can optionally be
 *   summarized and injected back into the main chat as a user message.
 *
 *   Backed by a separate in-memory agent session (read/bash/edit/write tools
 *   only). Persists side-thread Q&A entries to the session log so reopening
 *   `/btw` restores history.
 *
 *   Source: adapted from mitsuhiko/agent-stuff.
 *
 * Use cases:
 *   - Quick exploratory questions ("what does this file do?") without
 *     derailing the main task.
 *   - Planning / brainstorming next steps with awareness of current context.
 *   - Cheap lookups (find a symbol, summarize a diff) that you don't want in
 *     the main transcript.
 *   - Pulling a refined summary back into the main chat as a handoff.
 *
 * Common usage patterns:
 *   - `/btw` — open the side popover; pick "Continue" or "Start fresh" if a
 *     prior thread exists.
 *   - `/btw what does buildSessionContext do?` — ask immediately.
 *   - Type follow-ups in the popover input; Enter submits, Esc closes.
 *   - On close with a non-empty thread, choose "Inject summary into main
 *     chat" to hand findings back to the primary session.
 */
