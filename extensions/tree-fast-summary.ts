 ...
 * Tree Fast Summary
 *
 * `/tree` branch summarization (and `/compact`) normally run on the active
 * session model. This extension instead routes the summary through a cheap,
 * fast model so navigating the tree doesn't burn your expensive main model.
 *
...
async function readFastSpecFrom(filePath: string): Promise<FastModelSpec | null> {
	try {
		...
		const spec = parsed?.modes?.[FAST_MODE_NAME] as { provider?: unknown; modelId?: unknown } | undefined;
	...
	pi.on("session_before_tree", async (event, ctx: ExtensionContext) => {
		...
		if (!auth.ok || !auth.apiKey) {
			if (ctx.hasUI) {
				...
				ctx.ui.notify(`tree-fast-summary: ${why} for ${model.provider}; using default summarizer`, "warning");
			}
			return;
		}
